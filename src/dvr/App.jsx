...
const EMPTY_FLEET = { drivers: [], asset_ids: [], trip_ids: [], events: [], fleet_id: null, client_id: null }
...
export default function App() {
  ...
  // Items picked from the @-mention dropdown in the message being composed.
  // Kept apart from chips on purpose: chips are filters and mostly never came
  // from @ (server-restored filters, "Use trip", the date picker). Only an @
  // pick may turn a send into autocomplete_result, so it's tracked here and
  // cleared after every send — a chip carried into the next message is a
  // filter, not a fresh pick.
  const atPicksRef = useRef([])
  ...
  function handleServerMessage(msg) {
    ...
    if (msg.type === 'interrupt') {
      ...
      if (p.message === 'show_results') {
        ...
        // don't repost the summary on every interrupt.
  ...
  const loadFleet = useCallback(
    async (fleetId, clientId) => {
      // Fail loudly and locally. Passing a non-string here (a click's
      // SyntheticEvent, or a whole fleet object) used to sail through the
      // template literal as "[object Object]" and come back as an opaque 400.
      if (typeof fleetId !== 'string' || typeof clientId !== 'string') {
        throw new Error(
          `loadFleet: expected strings, got fleetId=${typeof fleetId}, clientId=${typeof clientId}`,
        )
      }

      // Fleet stays in the path; the client rides along as a query param named
      // `clientId` (camelCase), matching what the backend's other endpoints
      // already read. The WS message below uses snake_case `client_id` to sit
      // beside `fleet_id` — the mismatch is deliberate, each transport keeps
      // its own convention.
      const url = `/${encodeURIComponent(fleetId)}/load-data?clientId=${encodeURIComponent(clientId)}`
      const res = await fetch(url)

      if (!res.ok) {
        // Read the body before falling back, so a bad/absent client ID reads
        // as itself instead of a generic failure.
        let serverMsg = ''
        try {
        ...
        if (res.status === 401 || res.status === 403) {
          throw new Error(serverMsg || `No access to client ID "${clientId}"`)
        }
        if (res.status === 400) {
          throw new Error(serverMsg || `Bad request for client ID "${clientId}"`)
        }
        throw new Error(serverMsg || `Bad response (${res.status})`)
      }
      ...
      setFleetData(next)
      // `next` already carries fleet_id + client_id, and it is what the
      // reconnect handler re-sends verbatim, so the socket keeps both after a
      // drop without the user reloading.
      send({ type: 'load_data', fleet_data: next, thread_id: threadId })
    },
    [send, threadId],
  )
  ...
  // announce: whether to also post the intro chat messages — true for a turn's
  // first show_results, false for a later show_results in the same turn (e.g.
  // after a chained ask-timestamp prompt), so re-renders don't spam duplicate
  // chat messages while still always updating what's shown in the panel.
  function renderTripResults(trips, sum, announce = true) {
    setCurrentTrips(trips)
    setSummary(sum || '')
    if (!announce) return
    // Say only what the backend actually reported. This used to append
    // "Results are shown in the panel." to every summary, which contradicted
    // itself on an empty result — "No trips found for the selected filters
    // (...). Results are shown in the panel." The panel renders its own
    // "No trips found" empty state, so the chat doesn't need to narrate it.
    const count = trips.length
    appendBot(sum || (count > 0 ? `Found ${count} trip${count === 1 ? '' : 's'}.` : 'No trips found for the selected filters.'))
    if (count > 0) {
  ...
  // The exact selectedItem the backend expects for each @-pickable option.
  // Built field by field so nothing extra from the source object leaks in (a
  // driver pick's raw record carries more than driverId/driverName).
  function pickSelectedItem(e) {
    if (e.option === 'Drivers') return { driverId: e.selectedItem.driverId, driverName: e.selectedItem.driverName }
    if (e.option === 'Assets') return { assetId: e.selectedItem.assetId }
    if (e.option === 'Trips') return { tripId: e.selectedItem.tripId }
    if (e.option === 'Event Types') return { event_type: e.selectedItem.event_type }
    return null
  }

  // The send type depends on @ picks made in THIS message, never on chips.
  // Chips used to decide it, so a server-restored date chip turned every plain
  // follow-up into autocomplete_result with no selectedItem, which the backend
  // rejects ("No item selected."). Chip context still reaches the backend on
  // every path as the text tags already prefixed onto `pt`.
  function dispatch(pt) {
    // Ignore a pick whose chip the user removed before sending.
    const picks = atPicksRef.current.filter((p) =>
      collectedRef.current.some((e) => e.option === p.option && chipKey(e) === chipKey(p)),
    )
    atPicksRef.current = []
    if (picks.length > 0) {
      const pick = picks[picks.length - 1] // the backend takes one item per message
      send({
        ...
        query_type: 'directed',
        option: pick.option,
        selectedItem: pickSelectedItem(pick),
        thread_id: threadId,
      })
    } else {
  ...
  function chatSend(text) {
    ...
    if (graphPaused) {
      // resume_graph ignores @ picks, but they still belong to this message
      // only. Clear them so they can't turn the next send into
      // autocomplete_result once the graph resumes.
      atPicksRef.current = []
      appendUser(t, chips)
      setTyping(true)
  ...
  // Called only by the @-mention dropdown: records the pick for this message,
  // then stages its chip. Other chip sources (server filters, "Use trip", the
  // date picker) write through commitCollected directly, so they never count as
  // picks. Don't call this from anywhere else, or the chip-decides-send-type
  // bug comes back. The pick is recorded even when the chip already exists,
  // since picking it again still means "this message is about that item".
  const onAtPick = useCallback(
    (entry) => {
      // There's no DateRange option on the backend, so a date is never a pick.
      if (entry.option !== 'DateRange') atPicksRef.current = [...atPicksRef.current, entry]
  ...
  // Editing either ID makes the current conversation meaningless: the backend
  // keys its per-connection state on fleet + client, and anything already on
  // screen belongs to the account being navigated away from. Clearing the
  // loaded fleet too keeps the reconnect handler from re-seeding the socket
  // with the old account. Guarded so this fires once rather than on every
  // keystroke — after the first edit there is nothing left to clear.
  function resetForIdChange() {
    if (messages.length === 0 && !fleetData.fleet_id) return
    newThread()
    setFleetData(EMPTY_FLEET)
  }
  ...
  return (
    <div className="shell">
      <TopBar connected={connected} onLoadFleet={loadFleet} onNewThread={newThread} onIdsChanged={resetForIdChange} />
      <div className="main">
